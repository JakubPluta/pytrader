...
def get_data_bars(symbols, timeframe='5Min',  slow=30, fast=5):
    data = pyt.client.get_barset(symbols, timeframe, limit=20).df
...
def get_signal_bars(symbol_list, rate, ema_slow, ema_fast):
    data = pyt.client.get_data_bars(symbol_list, rate, ema_slow, ema_fast)
    signals = {}
...
def time_to_open(current_time):
    ...
    next_day = datetime.datetime.combine(d, datetime.time(9, 30, ))
...
def run(stocks):
    ...
    while True:
        if 0 <= datetime.datetime.now().weekday() <= 4:
            print("Trading Session!")
            if datetime.time(9, 30) < datetime.datetime.now().time() <= datetime.time(15, 30):
                signals = get_signal_bars(stocks, '5Min', 20, 5)
                for signal in signals:
                    if signals[signal] == 1:
                        if signal not in [x.symbol for x in api.list_positions()]:
                            logging.warning('{} {} - {}'.format(datetime.datetime.now().strftime("%x %X"), signal,
                                                                signals[signal]))
                ...
                print('Market closed ({})'.format(datetime.datetime.now()))
                print('Sleeping', round(time_to_open(datetime.datetime.now())/60/60, 2), 'hours')
                time.sleep(time_to_open(datetime.datetime.now()))
